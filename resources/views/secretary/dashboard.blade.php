    ...
    @section('content')
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div id="dashboardGrid" class="grid grid-cols-1 md:grid-cols-2 gap-4 transition-all duration-300">
                        <!-- Left Column: Records Dashboard -->
                        <div id="recordsCol">
                            <div class="p-2 text-gray-900 text-xl font-semibold">
                                {{ __('RECORDS DASHBOARD') }}
                            </div>
                            <div class="p-6">
                                <button id="toggleRecordsDashboard" class="mb-2 text-xl px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition">Show/Hide</button>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6" id="RecordsDashboard" style="display:none;">
                        ...
                        <!-- Right Column: Schedules Record -->
                        <div id="schedulesCol">
                            <div class="p-2 text-gray-900 text-xl font-semibold ">
                                <span>SCHEDULES RECORD</span>
                                <div class="mt-4">
                                    <button id="toggleSchedulesBtn" class="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition">View / Close</button>
                                </div>
                            </div>
                            <div class="p-6" id="SchedulesTable" style="display:none;">
                                @php
                                ...
                                @else
                                    <table class="min-w-full table-auto border border-gray-300 text-xs md:text-sm">
                                        <thead class="bg-gray-100">
                                            <tr>
                                                <th class="px-2 py-1 border-t border-l border-r">Event</th>
                                                <th class="px-2 py-1 border-t border-r">User</th>
                                                <th class="px-2 py-1 border-t border-r">Date</th>
                                                <th class="px-2 py-1 border-t border-r">Time</th>
                                                <th class="px-2 py-1 border-t border-r">People</th>
                                                <th class="px-2 py-1 border-t">Status</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach($schedules as $schedule)
                                                <tr>
                                                    <td class="px-2 py-1 border-l border-r">{{ $schedule->event->name ?? '-' }}</td>
                                                    <td class="px-2 py-1 border-r">{{ $schedule->user->name ?? '-' }}</td>
                                                    <td class="px-2 py-1 border-r">{{ $schedule->reservation_date }}</td>
                                                    <td class="px-2 py-1 border-r">{{ $schedule->reservation_time }}</td>
                                                    <td class="px-2 py-1 border-r">{{ $schedule->number_of_people }}</td>
                                                    <td class="px-2 py-1">{{ ucfirst($schedule->status) }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const recordsCol = document.getElementById('recordsCol');
                const schedulesCol = document.getElementById('schedulesCol');
                let dashboardVisible = false; // Hide by default
                function setGridLayout() {
                    if (dashboardVisible) {
                        dashboard.style.display = '';
                        grid.classList.remove('md:grid-cols-2');
                        grid.classList.add('grid-cols-1');
                        // Move schedulesCol below recordsCol
                        grid.appendChild(recordsCol);
                        grid.appendChild(schedulesCol);
                    } else {
                ...
                // Schedules table toggle
                const schedulesTable = document.getElementById('SchedulesTable');
                const toggleSchedulesBtn = document.getElementById('toggleSchedulesBtn');
                toggleSchedulesBtn.addEventListener('click', function() {
                    schedulesTable.style.display = schedulesTable.style.display === 'none' ? '' : 'none';
                });
            });
        </script>
    @endsection
